...
export const Calendar = ({ performances }: CalendarProps) => {
  const [selectedDate, setSelectedDate] = useState<string | null>(null);
  
  // 현재 년과 월을 상태로 관리
  const [currentYear, setCurrentYear] = useState<number>(new Date().getFullYear());
  const [currentMonth, setCurrentMonth] = useState<number>(new Date().getMonth()); // 0부터 시작
  
  // 월 변경 함수
  const handlePrevMonth = () => {
    setSelectedDate(null); // 선택된 날짜 초기화
    if (currentMonth === 0) {
      // 1월에서 이전 월로 이동하면 전년도의 12월로 변경
  ...
  const thisMonthPerformances = performances.filter(performance => {
    ...
    return performanceDate.getFullYear() === currentYear && performanceDate.getMonth() === currentMonth;
  });

  // 공연 날짜 목록 (중복 제거)
  const performanceDates = Array.from(new Set(thisMonthPerformances.map(performance => new Date(performance.date).getDate())));
  ...
  // 선택된 날짜의 공연들
  const selectedDatePerformances = thisMonthPerformances.filter(performance => {
    return performance.date.startsWith(selectedDate || '');
  });
  ...
  const getSelectedDay = () => {
    if (selectedDate) {
      ...
      const day = date.getDate(); // 날짜의 일(Day) 부분 추출
      return `${day}일 공연`;
    }
    return '';
  };
            ...
            >
              <div
                ...
                className={`cursor-pointer w-10 h-10 mx-auto flex items-center justify-center rounded-full
                  ${isSelected ? 'bg-blue-500 text-white' : hasPerformance ? 'border-2 border-blue-500 text-blue-600 font-bold' : 'text-gray-800'}
                `}
              >
                ...
                <Card key={performance.id} className="p-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex-1 text-left">
                      <h4 className="font-bold text-sm">{performance.title}</h4>
                      <p className="text-gray-600 text-sm">{performance.location}</p>
                      <p className="text-gray-500 text-sm">
                        {new Date(performance.date).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })}
                      </p>
                    </div>
                    {/* 오른쪽: 썸네일 이미지 */}
